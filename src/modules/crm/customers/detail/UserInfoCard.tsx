  ...
  useObserver(() => {
    ...
    useEffect(() => {
      if (userDetail) {
        ...
        crmStore.getCustomerInfo("000198");
      }
    }, [])

    useEffect(() => {
      if (crmStore.detailCustomer) {
        renderUserImg();
      }
    }, [crmStore.detailCustomer])
    // ** render user img
    const renderUserImg = () => {
      ...
      return (
        <Avatar
          ...
          content={crmStore.detailCustomer?.FullName ?? "Người dùng"}
      ...
      MySwal.fire({
        ...
        if (result.isConfirmed) {
          const param = {
            ...
            LeadID: crmStore.detailCustomer?.LeadID,
            ...
            >
              ...
              <div className="d-flex flex-column align-items-center text-center">
                ...
                <div className="user-info mb-1">
                  ...
                  <h4 className="mb-0">
                    {crmStore.detailCustomer?.FullName}
                  </h4>
                  <CardText tag="span">
                    <span style={{ color: `${crmStore.detailCustomer?.StatusColor}` }}>{crmStore.detailCustomer?.StatusName}</span>
                  </CardText>
                </div>
              </div>
              ...
              <div className="info-container">
                <ListInfo className="list-unstyled custom-list-unstyled">
                  <ListItem className="mb-75 d-flex">
                    ...
                    <ListItemValue className="d-flex justify-content-end">{crmStore.detailCustomer?.Phone01} <br /> {crmStore.detailCustomer?.Phone02}</ListItemValue>
                  </ListItem>
                  <ListItem className="mb-75 d-flex">
                  ...
                  {/* <ListItem className="mb-75 d-flex">
                    ...
                    <ListItemValue className="d-flex justify-content-end">{crmStore.detailCustomer?.ProfileName}</ListItemValue>
                  </ListItem>
                  <ListItem className="mb-75 d-flex">
                  ...
                  <ListItem className="mb-75 d-flex">
                    ...
                    <ListItemValue className="d-flex justify-content-end">{crmStore.detailCustomer?.BranchID}</ListItemValue>
                  </ListItem>
                  <ListItem className="mb-75 d-flex">
                    <ListItemValue className="d-flex fw-bolder me-25">Ngày tạo</ListItemValue>
                    <ListItemValue className="d-flex justify-content-end">{Moment.formatDateNew(crmStore.detailCustomer?.CreatedDate, "HH:mm - DD/MM/yyyy")}</ListItemValue>
                  </ListItem>
                  <ListItem className="mb-75 d-flex">
                    <ListItemValue className="d-flex fw-bolder me-25">Ngày cập nhật cuối</ListItemValue>
                    <ListItemValue className="d-flex justify-content-end">{Moment.formatDateNew(crmStore.detailCustomer?.UpdatedDate, "HH:mm - DD/MM/yyyy")}</ListItemValue>
                  </ListItem> */}
                </ListInfo>
                {/* <Collapse className="custom-collapse" ghost expandIconPosition="end">
                  <Panel header="Thông tin mở rộng" key="1">
                    <ListInfo className="list-unstyled">
                      {crmStore.detailCustomer?.LeadAttribute.map((item: any) => (
                        <ListItem className="mb-75 d-flex">
              ...
              <div className="d-flex justify-content-center pt-2">
                {/* <Tooltip placement="left" title={"Cập nhật"}>
                  ...
                  >
                    <Link
                        to={{ pathname: `/crm/potential-customer/edit/${crmStore.detailCustomer?.LeadID}`, state: { from: 'detail' } }}
