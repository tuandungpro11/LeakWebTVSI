...
const FormUpdate = (valueUpdate: any) =>
  useObserver(() => {
    ...
    const ClosePopup = () => {
      appAuStore.isShowPopupModalUpdate = false;
    };
    const onUpdApp = () => {
      const valueForm = form.getFieldsValue();
      const param = {
        UserName: appStore.account.LoginName,
        AppID: valueBind.AppID,
        AppName: valueForm.appName.trim(),
        Description: valueForm.descript == undefined?"":valueForm.descript.trim(),
      };
      appAuStore.UpdateApp(param);
    };

    useEffect(() => {
      form.setFieldsValue({
        appName: valueBind.AppName,
        descript: valueBind.Description,
      });
    }, []);
